...
public class MainActivity38 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("leq1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("leq2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("leq3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("leq4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("leq5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("leqprob.pdf").load();
        }

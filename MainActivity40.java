...
public class MainActivity40 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("quad1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("quad2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("quad3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("quad4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("quad5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("quad6.pdf").load();
        }
        if(lesposition==6){
            pdfView.fromAsset("quadprobm.pdf").load();
        }

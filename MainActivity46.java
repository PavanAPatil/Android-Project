...
public class MainActivity46 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Probability1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Probability2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Probability3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Probability4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Probability5.pdf").load();
        }

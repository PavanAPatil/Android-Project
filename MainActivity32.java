...
public class MainActivity32 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("pschap1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("pschap2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("pschap3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("pschap4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("pschap5.pdf").load();
        }

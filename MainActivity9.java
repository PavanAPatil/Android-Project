...
public class MainActivity9 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Eng2.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Eng2.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Eng2.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Eng2.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Eng2.5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("Eng2.6.pdf").load();
        }
    }
    @Override

...
public class MainActivity18 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("mar5.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("mar6.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("mar7.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("mar8.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("mar8.1.pdf").load();
        }
    }
    @Override

...
public class MainActivity113 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("regulations.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("bridgeco.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("englishteach.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("marathiteach.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("maths1teach.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("maths2teach.pdf").load();
        }
        if(lesposition==6){
            pdfView.fromAsset("sci1teach.pdf").load();
        }
        if(lesposition==7){
            pdfView.fromAsset("sci2teach.pdf").load();
        }
        if(lesposition==8){
            pdfView.fromAsset("historyteach.pdf").load();
        }
        if(lesposition==9){
            pdfView.fromAsset("geogteach.pdf").load();
        }
    }
    @Override

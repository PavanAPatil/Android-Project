...
public class MainActivity52 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("sim1.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("sim1.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("sim1.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("sim1.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("simprob.pdf").load();
        }

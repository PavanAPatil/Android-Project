...
public class MainActivity16 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("mar1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("mar21.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("mar22.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("mar3.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("mar4.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("mar4.1.pdf").load();
        }
    }
    @Override

...
public class MainActivity22 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("mar13.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("mar14.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("mar15.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("mar16.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("mar161.pdf").load();
        }
    }
    @Override

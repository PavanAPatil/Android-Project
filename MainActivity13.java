...
public class MainActivity13 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Eng4.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Eng4.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Eng4.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Eng4.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Eng4.5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("Eng4.6.pdf").load();
        }
    }
    @Override

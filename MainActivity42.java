...
public class MainActivity42 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Ap1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Ap2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Ap3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Ap4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Approblem.pdf").load();
        }

...
public class MainActivity11 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Eng3.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Eng3.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Eng3.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Eng3.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Eng3.5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("Eng3.6.pdf").load();
        }
    }
    @Override

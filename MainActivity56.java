...
public class MainActivity56 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("circ3.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("circ3.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("circ3.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("circ3.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("circ3.5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("circprob.pdf").load();
        }

...
public class MainActivity64 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("mens1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("mens2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("mens3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("mens4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("mensprobm.pdf").load();
        }
    }
    @Override

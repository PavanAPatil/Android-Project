...
public class MainActivity44 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("Fp1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("Fp2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("Fp3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("Fp4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("Fpprob1.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("Fpprob2.pdf").load();
        }

...
public class MainActivity20 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("mar9.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("mar10.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("mar11.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("mar12.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("mar121.pdf").load();
        }
    }
    @Override

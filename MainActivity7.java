...
public class MainActivity7 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(lesposition==0){
            pdfView.fromAsset("English 1.1.pdf").load();
        }
        if(lesposition==1){
            pdfView.fromAsset("English 1.2.pdf").load();
        }
        if(lesposition==2){
            pdfView.fromAsset("English 1.3.pdf").load();
        }
        if(lesposition==3){
            pdfView.fromAsset("English 1.4.pdf").load();
        }
        if(lesposition==4){
            pdfView.fromAsset("English 1.5.pdf").load();
        }
        if(lesposition==5){
            pdfView.fromAsset("English 1.6.pdf").load();
        }
    }
    @Override
